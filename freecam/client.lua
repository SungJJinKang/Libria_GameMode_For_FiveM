...
function SetEnableFreeCam(enable)

    if enable then

          
    camMenu = NativeUI.CreateMenu('Spectate', '', 0.5, 0.8)
    camMenu._Visible = false
    ...
    camMenu.Controls.Back.Enabled = false
    
        itemCamPrecision = 1.0
        itemCamFov = 50.0
        isAttached = false

        StartFreeCam(itemCamFov)

        SetCamFov(cam, itemCamFov)
    ...
    if (cam) then
        ...
        if (isAttached) then
            ...
            if (IsDisabledControlPressed(1, 32)) then -- W
                ...
                x = x - (0.1 * 1 * multCoordX)
                y = y + (0.1 * 1 * multCoordY)
            end
            ...
            if Cfg.useRot == true then

                if (IsDisabledControlPressed(1, 44)) then -- Q
                    offsetRotY = offsetRotY - (1.0 * 1)
